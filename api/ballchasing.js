export default async function handler(req, res) {
  ...
  try {
    if (replayId) {
      const replay = await ballFetch(`https://ballchasing.com/api/replays/${encodeURIComponent(replayId)}`);
    ...
    const listData = await ballFetch(`https://ballchasing.com/api/replays?${search.toString()}`);
    ...
    const replay = await ballFetch(`https://ballchasing.com/api/replays/${encodeURIComponent(first.id)}`);
